...
import { Plus, Pencil, Trash2, CheckCircle, DollarSign } from "lucide-react"
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Record a payment for rental {paymentRental?.rentalNumber}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {paymentRental && (
              <>
                <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{paymentRental.customer.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Total Amount:</span>
                    <span className="font-medium">{formatCurrency(paymentRental.totalAmount)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Already Paid:</span>
                    <span className="font-medium text-green-600">
                      {formatCurrency(getTotalPaid(paymentRental.payments))}
                    </span>
                  </div>
                  <div className="flex justify-between border-t pt-2">
                    <span className="text-sm font-semibold">Balance Due:</span>
                    <span className="font-bold text-orange-600">
                      {formatCurrency(paymentRental.totalAmount - getTotalPaid(paymentRental.payments))}
                    </span>
                  </div>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="paymentAmount">Payment Amount (FCFA) *</Label>
                  <Input
                    id="paymentAmount"
                    type="number"
                    step="0.01"
